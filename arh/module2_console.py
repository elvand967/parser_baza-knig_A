...
def main(n, x):
    ...
    log_filename = f"D:\Python\myProject\parser_baza-knig\log_files\id_n{n}_x{x}_module2_console.py_{current_time}_.log"

    # Открываем лог-файл для записи
    with open(log_filename, 'w') as log_file:
        log_file.write(f"Лог-файл для id={n} - {x}")
    ...
    with open(log_filename, 'a') as log_file:
        ...
        print(f"Загружены данные из JSON. ({file_path})")
        sys.stdout.flush()
